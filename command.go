...
//
// args is []patterns.ProcessArg because the vector flows verbatim into
// patterns.Subprocess (and ultimately os/exec), whose signature this wrapper
// does not control.
...
// without executing git. The tokens are rebound to []patterns.ProcessArg because
...
func gitWith(run runner, args ...GitArg) gloo.Command[[]byte, []byte] {
	argv := make([]patterns.ProcessArg, len(args))
	for i, a := range args {
		argv[i] = patterns.ProcessArg(a)
	}
	return run("git", argv...)
}
